...
-- FITUR TOGGLE VISIBILITY (BARU)
_G.ShowAimBtn = true
_G.ShowRayfieldBtn = true
...
-- [[ 2. STATIC AIM BUTTON (UPDATE: TENGAH KANAN LAYAR) ]]
local ScreenGui = Instance.new("ScreenGui", game.CoreGui)
local AimBtn = Instance.new("TextButton", ScreenGui)
AimBtn.Name = "AimButtonCenterRight"
AimBtn.Size = UDim2.new(0, 70, 0, 70) 
AimBtn.AnchorPoint = Vector2.new(1, 0.5)
AimBtn.Position = UDim2.new(1, -20, 0.5, 0) 

AimBtn.BackgroundColor3 = Color3.fromRGB(255, 50, 50); AimBtn.Text = "AIM\nOFF"; AimBtn.TextColor3 = Color3.new(1,1,1)
AimBtn.Font = Enum.Font.SourceSansBold; AimBtn.TextSize = 15; Instance.new("UICorner", AimBtn).CornerRadius = UDim.new(1, 0)
local UIStroke = Instance.new("UIStroke", AimBtn); UIStroke.Thickness = 3; UIStroke.Color = Color3.new(0,0,0)

local function UpdateAimBtnVisibility()
    AimBtn.Visible = _G.ShowAimBtn
end
...
AimBtn.MouseButton1Click:Connect(function() 
    ...
    -- Pengaman: Jika tombol Rayfield di-hide dan menu tertutup, klik tombol AIM akan memunculkan menu kembali
    if not _G.ShowRayfieldBtn then
...
-- [[ FUNGSI HIDE/SHOW TOMBOL RAYFIELD AUTOMATIC ]]
local function SetRayfieldBtnVisibility(Value)
    _G.ShowRayfieldBtn = Value
    for _, gui in pairs(game.CoreGui:GetChildren()) do
        if gui:IsA("ScreenGui") then
            local btn = gui:FindFirstChild("TextButton") or gui:FindFirstChild("Button")
            if btn and (btn.Text == "Show Rayfield" or string.find(string.lower(btn.Text), "rayfield")) then
                btn.Visible = Value
            end
        end
    end
...
TabComb:CreateToggle({Name = "Show/Hide Aim Button", CurrentValue = true, Callback = function(V) _G.ShowAimBtn = V; UpdateAimBtnVisibility() end})
...
TabMisc:CreateToggle({Name = "Show/Hide Rayfield Button", CurrentValue = true, Callback = function(V) SetRayfieldBtnVisibility(V) end})
